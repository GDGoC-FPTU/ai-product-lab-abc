...
def rule_based_fallback(user_input: str) -> str:
    """
    Deterministic safety fallback used when Gemini is unavailable.

    Critical operational boundaries are enforced in code so the workflow
    remains safe even when the API key, network, SDK, or model is unavailable.
    """
    battery_match = re.search(r"(\d+)\s*%", user_input)
    distance_match = re.search(r"(\d+)\s*km", user_input, re.IGNORECASE)
            ...
            return json.dumps(
                {
                    ...
                    "reason": (
                        "Battery level under critical threshold of 5%. "
    ...
    return (
        "[DRAFT_ONLY] Yêu cầu đã được ghi nhận và đang chờ "
        "điều phối viên kiểm tra, phê duyệt."
    )
...
def evaluate_prompt(user_input: str) -> str:
    ...
    Supports either the new google-genai SDK or the legacy
    google-generativeai SDK.
    """
    api_key = os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")

    # Enforce the critical battery rule before calling the LLM.
    safe_fallback = rule_based_fallback(user_input)
    if "dispatch_mobile_charger" in safe_fallback:
        return safe_fallback

    # CI and offline environments may not expose an API key.
    if not api_key:
        return safe_fallback
    ...
    try:
        ...
        try:
            ...
            output = response.text or safe_fallback
            if not output.startswith("[DRAFT_ONLY]"):
                output = f"[DRAFT_ONLY] {output}"
            return output
        except Exception as error:
            print(
                f"[WARN] Gemini unavailable; using rule-based fallback: {error}",
                file=sys.stderr,
            )
            return safe_fallback
...
if __name__ == "__main__":
    ...
    if not api_key:
        print(
            "\033[93m[WARN] GEMINI_API_KEY is not set; "
            "running deterministic safety fallback.\033[0m"
        )
